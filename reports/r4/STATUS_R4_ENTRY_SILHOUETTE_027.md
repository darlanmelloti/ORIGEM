# STATUS R4 — Forest Entry Silhouette 027

**Entrega:** `DEV4-R4-FOREST-ENTRY-SILHOUETTE-027`
...
A entrega introduz `ForestEntrySilhouette.gd` como camada R4 modular e estática. O módulo actua exclusivamente sobre **uma árvore R4 já existente e sem colisor associado** na faixa `Z=112–125`, anterior à clareira Orion. A transformação determinística reforça a silhueta de entrada sem inserir elementos na clareira, criar barreiras ambientais ou sistemas dinâmicos.
...
| Salvaguarda | Resultado da entrega 027 |
|---|---|
| Clareira Orion `Z≈126–151` | Não tocada; a faixa de trabalho termina em `Z=125` |
...
O parser Godot completou sem erro de script ou de carregamento. A prova dedicada `qa/regions/verify_r4_entry_silhouette.gd` concluiu com o marcador:

> `[ORIGEM_R4_ENTRY_SILHOUETTE_OK] entrada, clareira, corredor e física R4 preservados.`
...
Após a publicação desta entrega, o marcador operacional deve avançar para `DEV4-R4-FOREST-ORION-MARGIN-028`, mantendo as proteções da clareira Orion, do corredor e da transição Majestic, além da validação por parser, prova própria, porta R4 e as três rotas antes da publicação.
