# STATUS R4 — Forest Orion Return 031

**Entrega:** `DEV4-R4-FOREST-ORION-RETURN-031`
...
A entrega introduz `ForestOrionReturn.gd` como camada R4 modular e estática. O módulo actua exclusivamente sobre **uma copa focal R4 já existente e sem colisor associado** na faixa `Z=152–158`, imediatamente posterior à clareira Orion. A transformação determinística reforça a leitura de retorno sem inserir elementos em `Z≈126–151`, criar barreiras vegetais ou sistemas dinâmicos.
...
| Salvaguarda | Resultado da entrega 031 |
|---|---|
| Clareira Orion `Z≈126–151` | Não tocada; a faixa de trabalho começa em `Z=152` |
| Corredor e conector Majestic | Copa mantida a pelo menos `7,5 m` do eixo do trilho |
...
O parser Godot completou sem erro de script ou de carregamento. A prova dedicada `qa/regions/verify_r4_orion_return.gd` concluiu com o marcador:

> `[ORIGEM_R4_ORION_RETURN_OK] retorno, clareira, corredor e física R4 preservados.`
...
Após a publicação desta entrega, o marcador operacional deve avançar para `DEV4-R4-FOREST-MAJESTIC-MARGIN-032`, mantendo as proteções da clareira Orion, do corredor e da transição Majestic, além da validação por parser, prova própria, porta R4 e as três rotas antes da publicação.
