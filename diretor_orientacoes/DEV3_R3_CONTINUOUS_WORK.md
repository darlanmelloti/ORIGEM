...
```text
status: ACTIVE
task_id: DEV3-R3-ARCH-LATERAL-SILENCE-037
owner: Dev3
...
## Tarefa ativa — DEV3-R3-ARCH-LATERAL-SILENCE-037

A margem lateral do Arco deve manter um **silêncio material lateral sem reflexos**, limitado a materiais R3 já existentes e herdados de `r3_margin_calm_rest`. A tarefa deve conservar apenas a ausência de reflexão lateral sem criar trilho, texto, UI, placas, objetos, geometria, luzes, emissão, VFX, áudio ou colisores; a rota R2→R4 deve permanecer aberta.
...
| Material | Actuar somente em materiais laterais existentes já marcados `r3_margin_calm_rest`, sem criar trilho ou objectos novos |
...
`DEV3-R3-ARCH-LATERAL-SILENCE-037` preserva a margem lateral silenciosa sem alterar o corredor.
