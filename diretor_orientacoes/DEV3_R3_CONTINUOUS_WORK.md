...
```text
status: ACTIVE
task_id: DEV3-R3-ARCH-LATERAL-STILL-033
owner: Dev3
...
## Tarefa ativa — DEV3-R3-ARCH-LATERAL-STILL-033

A margem lateral do Arco deve manter uma **imobilidade lateral sem reflexos**, limitada a materiais R3 já existentes e herdados de `r3_margin_silence_rest`. A tarefa deve conservar apenas o acabamento lateral mate sem criar trilho, texto, UI, placas, objetos, geometria, luzes, emissão, VFX, áudio ou colisores; a rota R2→R4 deve permanecer aberta.
...
| Material | Actuar somente em materiais laterais existentes já marcados `r3_margin_silence_rest`, sem criar trilho ou objectos novos |
...
`DEV3-R3-ARCH-LATERAL-STILL-033` preserva a margem lateral imóvel sem alterar o corredor.
