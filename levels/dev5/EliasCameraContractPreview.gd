...
func _save_snapshot_qa(snapshot_path: String) -> void:
	...
	if viewport_texture == null:
		print("[DEV5_ELIAS_CAMERA] snapshot_unavailable=headless_renderer path=%s" % snapshot_path)
		return
	var image := viewport_texture.get_image()
	if image == null:
		print("[DEV5_ELIAS_CAMERA] snapshot_unavailable=headless_image path=%s" % snapshot_path)
		return
