...
func _build_final_dome_traversal_proxy() -> void:
	...
	if base_edge != null:
		...
		for route_x in [0.0]:
			...
			route_marker.position = Vector3(route_x, 0.36, -3.05)
			route_marker.scale = Vector3(0.34, 0.10, 0.30)
			route_marker.rotation = Vector3(0.03, 0.14 * sign(route_x), 0.02)
			var route_material := StandardMaterial3D.new()
			route_material.albedo_color = Color("#3a5a6e")
			route_material.roughness = 0.90
