## ArchMarginStillMatte.gd
## DEV3-R3-ARCH-MARGIN-STILL-060
## Imobilidade material da margem em superfícies existentes, sem criar geometria, texto, UI, luz, emissão, VFX, áudio ou colisão.
class_name R3ArchMarginStillMatte
extends Node

static func install(arch: Node3D) -> R3ArchMarginStillMatte:
	if arch == null:
		return null
	var existing: R3ArchMarginStillMatte = arch.get_node_or_null("R3ImobilidadeMateDaMargem060") as R3ArchMarginStillMatte
	if existing != null:
		return existing
	var layer := R3ArchMarginStillMatte.new()
	layer.name = "R3ImobilidadeMateDaMargem060"
...
func _ready() -> void:
	...
	for mesh_node in arch.find_children("*", "MeshInstance3D", true, false):
		...
		if material != null and mesh.has_meta("r3_lateral_calm_matte"):
			material.roughness = 1.0
			material.metallic = 0.0
			material.emission_enabled = false
			mesh.set_meta("r3_margin_still_matte", true)
