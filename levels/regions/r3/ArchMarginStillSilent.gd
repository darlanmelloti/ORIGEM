## ArchMarginStillSilent.gd
## DEV3-R3-ARCH-MARGIN-STILL-054
## Imobilidade material da margem em superfícies existentes, sem criar geometria, texto, UI, luz, emissão, VFX, áudio ou colisão.
class_name R3ArchMarginStillSilent
extends Node

static func install(arch: Node3D) -> R3ArchMarginStillSilent:
	if arch == null:
		return null
	var existing: R3ArchMarginStillSilent = arch.get_node_or_null("R3ImobilidadeSilenciosaDaMargem") as R3ArchMarginStillSilent
	if existing != null:
		return existing
	var layer := R3ArchMarginStillSilent.new()
	layer.name = "R3ImobilidadeSilenciosaDaMargem"
...
func _ready() -> void:
	...
	for mesh_node in arch.find_children("*", "MeshInstance3D", true, false):
		...
		if material != null and mesh.has_meta("r3_lateral_rest_silent"):
			material.roughness = 1.0
			material.metallic = 0.0
			material.emission_enabled = false
			mesh.set_meta("r3_margin_still_silent", true)
