## ArchLateralStillSilent.gd
## DEV3-R3-ARCH-LATERAL-STILL-051
## Imobilidade material lateral de superfícies existentes, sem criar geometria, texto, UI, luz, emissão, VFX, áudio ou colisão.
class_name R3ArchLateralStillSilent
extends Node

static func install(arch: Node3D) -> R3ArchLateralStillSilent:
	if arch == null:
		return null
	var existing: R3ArchLateralStillSilent = arch.get_node_or_null("R3ImobilidadeLateralSilenciosa") as R3ArchLateralStillSilent
	if existing != null:
		return existing
	var layer := R3ArchLateralStillSilent.new()
	layer.name = "R3ImobilidadeLateralSilenciosa"
...
func _ready() -> void:
	...
	for mesh_node in arch.find_children("*", "MeshInstance3D", true, false):
		...
		if material != null and mesh.has_meta("r3_margin_rest_silent"):
			material.roughness = 1.0
			material.metallic = 0.0
			material.emission_enabled = false
			mesh.set_meta("r3_lateral_still_silent", true)
