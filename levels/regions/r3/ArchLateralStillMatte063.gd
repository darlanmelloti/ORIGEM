## ArchLateralStillMatte063.gd
## DEV3-R3-ARCH-LATERAL-STILL-063
## Imobilidade material lateral em superfícies existentes, sem criar geometria, texto, UI, luz, emissão, VFX, áudio ou colisão.
class_name R3ArchLateralStillMatte063
extends Node

static func install(arch: Node3D) -> R3ArchLateralStillMatte063:
	if arch == null:
		return null
	var existing: R3ArchLateralStillMatte063 = arch.get_node_or_null("R3ImobilidadeMateLateral063") as R3ArchLateralStillMatte063
	if existing != null:
		return existing
	var layer := R3ArchLateralStillMatte063.new()
	layer.name = "R3ImobilidadeMateLateral063"
...
func _ready() -> void:
	...
	for mesh_node in arch.find_children("*", "MeshInstance3D", true, false):
		...
		if material != null and mesh.has_meta("r3_margin_calm_matte_062"):
			material.roughness = 1.0
			material.metallic = 0.0
			material.emission_enabled = false
			mesh.set_meta("r3_lateral_still_matte_063", true)
