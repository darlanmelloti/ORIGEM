## ArchLateralSilenceRest.gd
## DEV3-R3-ARCH-LATERAL-SILENCE-037
## Silêncio material lateral de superfícies existentes, sem criar geometria, texto, UI, luz, emissão, VFX, áudio ou colisão.

class_name R3ArchLateralSilenceRest
extends Node

static func install(arch: Node3D) -> R3ArchLateralSilenceRest:
	if arch == null:
		return null
	var existing: R3ArchLateralSilenceRest = arch.get_node_or_null("R3SilencioLateralEstavel") as R3ArchLateralSilenceRest
	if existing != null:
		return existing
	var layer := R3ArchLateralSilenceRest.new()
	layer.name = "R3SilencioLateralEstavel"
...
func _ready() -> void:
	...
	for mesh_node in arch.find_children("*", "MeshInstance3D", true, false):
		...
		if material != null and mesh.has_meta("r3_margin_calm_rest"):
			material.roughness = 1.0
			material.metallic = 0.0
			material.emission_enabled = false
			mesh.set_meta("r3_lateral_silence_rest", true)
