## ArchLateralCalmMatte065.gd
## DEV3-R3-ARCH-LATERAL-CALM-065
## Calma material lateral em superfícies existentes, sem criar geometria, texto, UI, luz, emissão, VFX, áudio ou colisão.
class_name R3ArchLateralCalmMatte065
extends Node

static func install(arch: Node3D) -> R3ArchLateralCalmMatte065:
	if arch == null:
		return null
	var existing: R3ArchLateralCalmMatte065 = arch.get_node_or_null("R3CalmaMateLateral065") as R3ArchLateralCalmMatte065
	if existing != null:
		return existing
	var layer := R3ArchLateralCalmMatte065.new()
	layer.name = "R3CalmaMateLateral065"
...
func _ready() -> void:
	...
	for mesh_node in arch.find_children("*", "MeshInstance3D", true, false):
		...
		if material != null and mesh.has_meta("r3_margin_rest_matte_064"):
			material.roughness = 1.0
			material.metallic = 0.0
			material.emission_enabled = false
			mesh.set_meta("r3_lateral_calm_matte_065", true)
