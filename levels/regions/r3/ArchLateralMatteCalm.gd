## ArchLateralMatteCalm.gd
## DEV3-R3-ARCH-LATERAL-MATTE-047
## Acabamento material mate lateral em superfícies existentes, sem criar geometria, texto, UI, luz, emissão, VFX, áudio ou colisão.
class_name R3ArchLateralMatteCalm
extends Node

static func install(arch: Node3D) -> R3ArchLateralMatteCalm:
	if arch == null:
		return null
	var existing: R3ArchLateralMatteCalm = arch.get_node_or_null("R3AcabamentoMateLateralCalmo") as R3ArchLateralMatteCalm
	if existing != null:
		return existing
	var layer := R3ArchLateralMatteCalm.new()
	layer.name = "R3AcabamentoMateLateralCalmo"
...
func _ready() -> void:
	...
	for mesh_node in arch.find_children("*", "MeshInstance3D", true, false):
		...
		if material != null and mesh.has_meta("r3_margin_still_calm"):
			material.roughness = 1.0
			material.metallic = 0.0
			material.emission_enabled = false
			mesh.set_meta("r3_lateral_matte_calm", true)
