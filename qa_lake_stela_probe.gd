...
func _run_probe() -> void:
	...
	var stela: StaticBody3D = scene.get_node_or_null("TempleLevel/RegiaoFlorestaLagoExploravel/RuinasSubmersasDoLago/RuneP0_02") as StaticBody3D
	if player == null or stela == null:
		print("[ORIGEM_QA_LAKE_STELA] FALHA: player=%s stela=%s" % [str(player), str(stela)])
		quit(2)
		return
	...
	if ray == null:
		print("[ORIGEM_QA_LAKE_STELA] FALHA: raio ausente")
		quit(3)
		return
	...
	var hud_success: bool = message_label != null and "RUNAS: Os Kharu vigiam" in message_label.text
	print("[ORIGEM_QA_LAKE_STELA] player=%s stela=%s alvo=%s raio=%s hud=%s texto=%s" % [player.global_position, stela.global_position, collider_name, str(ray_success), str(hud_success), message_label.text if message_label != null else "ausente"])
