...
inside_xvfb() {
  ...
  # A conclusão de gameplay é o marcador RESULT, não a permanência da janela X11.
  # Após uma margem curta para descarregar a telemetria, fecha-se apenas o processo
  # técnico da prova e valida-se o registo escrito pelo jogo.
  sleep 3
  kill "$pid" 2>/dev/null || true
  wait "$pid" || true
  pid=""
