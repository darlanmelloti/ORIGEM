...
xvfb-run -a -s '-screen 0 1600x900x24' bash -lc '
  ...
  sleep 2
  set +e
  ORIGEM_VALIDATION_TAKE=8 timeout 38s "'$GODOT'" --path "'$ROOT'" --rendering-method gl_compatibility --rendering-driver opengl3 "'$SCENE'" >"'$OUT'/gameplay.log" 2>&1
  GAME_STATUS=$?
  set -e
  wait "$CAP"
  if [[ "$GAME_STATUS" -ne 0 && "$GAME_STATUS" -ne 124 ]]; then
