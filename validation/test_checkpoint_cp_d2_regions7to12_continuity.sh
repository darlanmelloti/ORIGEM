...
if [[ "${MAP_MIRROR_VALIDATION:-0}" == "1" ]]; then
  ...
  for handoff_id in R07_R08 R08_R09 R09_R10 R10_R11 R11_R12; do
    grep -Eq "MAP_MIRROR_HANDOFF_CONTRACT id=${handoff_id} collider=.* authority=mapaorigem\\.webp physical=true" "$OUT/gameplay.log" || { echo "QA FAIL: physical handoff contract missing: ${handoff_id}" >&2; exit 1; }
  done
fi
