extends SceneTree
## CP-D2-R1R6-008 — captura dinâmica QA-only R3→R4.
## A rota é derivada exclusivamente de CartographicAnchors.gd.

const Anchors = preload("res://levels/CartographicAnchors.gd")

var qa_root: Node3D

func _initialize() -> void:
	qa_root = Node3D.new()
	qa_root.name = "CPD2R3R4CaptureQA"
	get_root().add_child(qa_root)
	var environment := WorldEnvironment.new()
	var world := Environment.new()
	world.background_mode = Environment.BG_COLOR
	world.background_color = Color("#a7bdc8")
	world.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
	world.ambient_light_color = Color("#cbd9de")
	world.ambient_light_energy = 0.95
	environment.environment = world
	qa_root.add_child(environment)
	var sun := DirectionalLight3D.new()
	sun.rotation_degrees = Vector3(-48.0, -32.0, 0.0)
	sun.light_energy = 1.15
	qa_root.add_child(sun)
	var ground := MeshInstance3D.new()
	var plane := PlaneMesh.new()
	plane.size = Vector2(170.0, 150.0)
	var ground_material := StandardMaterial3D.new()
	ground_material.albedo_color = Color("#385953")
	ground_material.roughness = 0.92
	plane.material = ground_material
	ground.mesh = plane
	ground.position = Vector3(-10.0, -0.06, 82.0)
	qa_root.add_child(ground)
	var start := Anchors.world_position(Anchors.ARCO_RUINAS, 0.0, 0.2)
	var finish := Anchors.world_position(Anchors.FLORESTA_DENSA_ENTRADA, 0.0, 0.2)
	_add_marker(start, "R3 ARCO DAS RUÍNAS", Color("#d8a2ff"))
	_add_marker(finish, "R4 FLORESTA DENSA", Color("#72e0a7"))
	var corridor := MeshInstance3D.new()
	var corridor_mesh := BoxMesh.new()
	corridor_mesh.size = Vector3(2.0, 0.10, start.distance_to(finish))
	var corridor_material := StandardMaterial3D.new()
	corridor_material.albedo_color = Color("#b7914f")
	corridor_mesh.material = corridor_material
	corridor.mesh = corridor_mesh
	corridor.position = (start + finish) * 0.5
	corridor.look_at_from_position(corridor.position, finish, Vector3.UP)
	qa_root.add_child(corridor)
	var camera := Camera3D.new()
	camera.name = "R3R4DynamicCamera"
	camera.current = true
	camera.fov = 78.0
	qa_root.add_child(camera)
	await process_frame
	var duration := 31.0
	var override_seconds := OS.get_environment("QA_CAPTURE_SECONDS")
	if not override_seconds.is_empty():
		duration = maxf(float(override_seconds), 1.0)
	var frame_count := maxi(30, ceili(duration * 10.0))
	var stable_target := start.lerp(finish, 0.5)
	for frame: int in range(frame_count):
		var t := float(frame) / float(frame_count - 1)
		var target := start.lerp(finish, t)
		camera.look_at_from_position(target + Vector3(-48.0, 36.0, -68.0), stable_target + Vector3(0.0, 2.0, 0.0), Vector3.UP)
		if frame % 30 == 0:
			print("R3R4_DYNAMIC progress=%0.2f" % t)
		await create_timer(duration / float(frame_count)).timeout
	print("CP-D2-R1R6-008_R3_R4_DYNAMIC_CAPTURE=PASS")
	quit(0)

func _add_marker(position: Vector3, title: String, color: Color) -> void:
	var marker := MeshInstance3D.new()
	var sphere := SphereMesh.new()
	sphere.radius = 0.9
	sphere.height = 1.8
	var material := StandardMaterial3D.new()
	material.albedo_color = color
	material.emission_enabled = true
	material.emission = color * 0.22
	sphere.material = material
	marker.mesh = sphere
	marker.position = position
	qa_root.add_child(marker)
	var label := Label3D.new()
	label.text = title
	label.position = position + (Vector3(-8.0, 4.8, 0.0) if title.begins_with("R4") else Vector3(-6.5, 3.2, 0.0))
	label.font_size = 72 if title.begins_with("R4") else 48
	label.pixel_size = 0.045 if title.begins_with("R4") else 0.032
	label.outline_size = 8
	label.modulate = Color("#f4f1e7")
	label.billboard = BaseMaterial3D.BILLBOARD_ENABLED
	qa_root.add_child(label)
