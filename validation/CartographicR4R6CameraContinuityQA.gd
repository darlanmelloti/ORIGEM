extends SceneTree
## CP-D2-R1R6-033 — câmara e corredor QA-only R4→R5→R6.
const Anchors = preload("res://levels/CartographicAnchors.gd")
const POINTS: Array[Vector2] = [Anchors.FLORESTA_DENSA_ENTRADA, Anchors.ACAMPAMENTO_MAJESTIC, Anchors.RUINAS_SUBMERSAS]
const NAMES: Array[String] = ["R4 FLORESTA DENSA", "R5 ACAMPAMENTO MAJESTIC", "R6 RUÍNAS SUBMERSAS"]
var qa_root: Node3D

func _initialize() -> void:
	qa_root = Node3D.new()
	qa_root.name = "CPD2R4R6CameraContinuityQA"
	...
	for index: int in range(POINTS.size()):
		...
		_add_marker(point, NAMES[index], Color("#70d0c0") if index < 2 else Color("#66cbd3"), index == 2)
		if index > 0:
			_add_segment(previous, point, Color("#70d0c0") if index == 1 else Color("#66cbd3"))
		previous = point
	...
	var center := Vector3(-14.0, 0.0, 182.0)
	for frame: int in range(frames):
		camera.look_at_from_position(center + Vector3(0.0, 300.0, 0.0), center, Vector3.FORWARD)
		if frame % 30 == 0:
			print("R4R6_CAMERA_DYNAMIC progress=%0.2f" % (float(frame) / float(frames - 1)))
		await create_timer(duration / float(frames)).timeout
	print("R4R6_CAMERA_LIGHT_BUDGET=PASS dynamic_lights=1 max=16")
	print("R4R6_CAMERA_SCOPE_GUARD=PASS R1-R6_dev1_readonly_standby")
	print("CP-D2-R1R6-033_R4_R5_R6_CAMERA_CORRIDOR=PASS")
	quit(0)
...
func _add_marker(point: Vector3, title: String, color: Color, larger: bool) -> void:
	...
	label.text = title
	label.position = point + Vector3(-20.0, 10.0, 0.0)
	label.font_size = 82 if larger else 68
	label.pixel_size = 0.054
...
func _add_segment(from_point: Vector3, to_point: Vector3, color: Color) -> void:
	...
	mesh.size = Vector3(2.5, 0.14, from_point.distance_to(to_point))
	var material := StandardMaterial3D.new()
	material.albedo_color = color
	material.emission_enabled = true
