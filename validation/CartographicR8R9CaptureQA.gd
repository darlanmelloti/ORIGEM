extends SceneTree
## CP-D2-R1R6-018 — captura dinâmica QA-only R8→R9.
## Deriva todas as posições de CartographicAnchors.gd.
...
func _initialize() -> void:
	...
	ground_material.albedo_color = Color("#3a5c52")
	ground_material.roughness = 0.92
	plane.material = ground_material
	ground.mesh = plane
	ground.position = Vector3(184.0, -0.06, 409.0)
	qa_root.add_child(ground)
	var start := Anchors.world_position(Anchors.OBSERVATORIO, 0.0, 0.2)
	var finish := Anchors.world_position(Anchors.TRILHA_MONTANHA_INICIO, 0.0, 0.2)
	_add_marker(start, "R8 OBSERVATÓRIO", Color("#f3c16c"), false)
	_add_marker(finish, "R9 TRILHA DA MONTANHA", Color("#8dd0e8"), true)
	...
	for frame: int in range(frame_count):
		...
		camera.look_at_from_position(target + Vector3(-42.0, 32.0, -56.0), stable_target + Vector3(0.0, 2.0, 0.0), Vector3.UP)
		if frame % 30 == 0:
			print("R8R9_DYNAMIC progress=%0.2f" % progress)
		await create_timer(duration / float(frame_count)).timeout
	print("CP-D2-R1R6-018_R8_R9_DYNAMIC_CAPTURE=PASS")
	quit(0)
