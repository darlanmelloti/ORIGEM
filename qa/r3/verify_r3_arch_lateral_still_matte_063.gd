...
func _run() -> void:
	...
	var layer: Node = arch.get_node_or_null("R3ImobilidadeMateLateral063") if arch != null else null
	if layer == null:
	...
	var marked: Array[Node] = arch.find_children("*", "MeshInstance3D", true, false).filter(func(node: Node) -> bool: return node.has_meta("r3_lateral_still_matte_063")) if arch != null else []
	if marked.is_empty():
	...
	for mesh_node: Node in marked:
		...
		if not mesh_node.has_meta("r3_margin_calm_matte_062"):
		...
		if material == null or material.roughness < 1.0 or material.metallic != 0.0 or material.emission_enabled:
			...
			break
	if issues.is_empty():
		print("[ORIGEM_R3_ARCH_LATERAL_STILL_MATTE_063_OK] imobilidade lateral preservada sem recursos adicionais")
		quit(0)
		return
	for issue: String in issues:
		printerr("[ORIGEM_R3_ARCH_LATERAL_STILL_MATTE_063_ERROR] %s" % issue)
	quit(1)
