...
func _run() -> void:
	...
	var marked: Array[Node] = arch.find_children("*", "MeshInstance3D", true, false).filter(func(node: Node) -> bool: return node.has_meta("r3_lateral_calm_stillness")) if arch != null else []
	if marked.is_empty():
		issues.append("nenhuma superfície lateral recebeu calma estável")
	for mesh_node: Node in marked:
		if not mesh_node.has_meta("r3_margin_matte_stillness"):
			issues.append("a calma lateral não herdou o acabamento mate da margem")
			break
	if issues.is_empty():
		print("[ORIGEM_R3_ARCH_LATERAL_CALM_STILLNESS_OK] calma lateral preservada sem recursos adicionais")
		quit(0)
		return
	for issue: String in issues:
		printerr("[ORIGEM_R3_ARCH_LATERAL_CALM_STILLNESS_ERROR] %s" % issue)
	quit(1)
