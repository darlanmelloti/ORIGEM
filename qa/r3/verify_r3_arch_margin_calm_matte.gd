...
func _run() -> void:
	...
	var layer: Node = arch.get_node_or_null("R3CalmaMateDaMargem") if arch != null else null
	if layer == null:
	...
	var marked: Array[Node] = arch.find_children("*", "MeshInstance3D", true, false).filter(func(node: Node) -> bool: return node.has_meta("r3_margin_calm_matte")) if arch != null else []
	if marked.is_empty():
	...
	for mesh_node: Node in marked:
		...
		if not mesh_node.has_meta("r3_lateral_matte_silent"):
		...
		if material == null or material.roughness < 1.0 or material.metallic != 0.0 or material.emission_enabled:
			...
			break
	if issues.is_empty():
		print("[ORIGEM_R3_ARCH_MARGIN_CALM_MATTE_OK] calma da margem preservada sem recursos adicionais")
		quit(0)
		return
	for issue: String in issues:
		printerr("[ORIGEM_R3_ARCH_MARGIN_CALM_MATTE_ERROR] %s" % issue)
	quit(1)
