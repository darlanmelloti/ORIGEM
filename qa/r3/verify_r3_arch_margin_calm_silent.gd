...
func _run() -> void:
	...
	var layer: Node = arch.get_node_or_null("R3CalmaSilenciosaDaMargem") if arch != null else null
	if layer == null:
		issues.append("camada de calma silenciosa da margem ausente")
	else:
	...
	var marked: Array[Node] = arch.find_children("*", "MeshInstance3D", true, false).filter(func(node: Node) -> bool: return node.has_meta("r3_margin_calm_silent")) if arch != null else []
	if marked.is_empty():
		issues.append("nenhuma superfície recebeu calma silenciosa da margem")
	for mesh_node: Node in marked:
		var mesh := mesh_node as MeshInstance3D
		var material := mesh.material_override as StandardMaterial3D if mesh != null else null
		if not mesh_node.has_meta("r3_lateral_still_silent"):
			issues.append("a calma da margem não herdou a imobilidade lateral")
			break
		if material == null or material.roughness < 1.0 or material.metallic != 0.0 or material.emission_enabled:
			...
			break
	if issues.is_empty():
		print("[ORIGEM_R3_ARCH_MARGIN_CALM_SILENT_OK] calma da margem preservada sem recursos adicionais")
		quit(0)
		return
	for issue: String in issues:
		printerr("[ORIGEM_R3_ARCH_MARGIN_CALM_SILENT_ERROR] %s" % issue)
	quit(1)
