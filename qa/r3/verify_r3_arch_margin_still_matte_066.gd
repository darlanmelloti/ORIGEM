...
func _run() -> void:
	...
	var layer: Node = arch.get_node_or_null("R3ImobilidadeMateDaMargem066") if arch != null else null
	if layer == null:
	...
	var marked: Array[Node] = arch.find_children("*", "MeshInstance3D", true, false).filter(func(n: Node) -> bool: return n.has_meta("r3_margin_still_matte_066")) if arch != null else []
	if marked.is_empty():
	...
	for node: Node in marked:
		...
		if not node.has_meta("r3_lateral_calm_matte_065") or mat == null or mat.roughness < 1.0 or mat.metallic != 0.0 or mat.emission_enabled:
			issues.append("herança ou material inválido")
			break
	if issues.is_empty():
		print("[ORIGEM_R3_ARCH_MARGIN_STILL_MATTE_066_OK] imobilidade da margem preservada sem recursos adicionais")
		quit(0)
		return
	for issue: String in issues:
		printerr("[ORIGEM_R3_ARCH_MARGIN_STILL_MATTE_066_ERROR] %s" % issue)
	quit(1)
