...
func _run() -> void:
	...
	if change_error != OK:
		printerr("[ORIGEM_R3_ARCH_ARRIVAL_CUE_ERROR] Não foi possível carregar o mundo principal.")
		quit(1)
		return
	await process_frame
	...
	if arch == null:
		...
		issues.append_array(ARCH_ARRIVAL_CUE_SCRIPT.call("validate", arch) as PackedStringArray)
		var cue: Node = arch.get_node_or_null("R3LeituraDeChegada")
		if cue != null:
			var contract: Dictionary = cue.call("get_contract") as Dictionary
			for prohibited_key: String in ["adds_geometry", "adds_lights", "adds_vfx", "adds_collision", "uses_ui"]:
				if bool(contract.get(prohibited_key, true)):
					issues.append("a leitura de chegada R3 declarou alteração proibida: %s" % prohibited_key)
		...
		if trigger == null:
			...
			if shape == null or shape.size.x < 5.50:
				issues.append("a leitura de chegada R3 reduziu a folga do gatilho central")
	if issues.is_empty():
		print("[ORIGEM_R3_ARCH_ARRIVAL_CUE_OK] Três grupos laterais reforçam a chegada R2→R3 sem UI, efeitos ou bloqueio.")
		quit(0)
		return
	for issue: String in issues:
		printerr("[ORIGEM_R3_ARCH_ARRIVAL_CUE_ERROR] %s" % issue)
	quit(1)
