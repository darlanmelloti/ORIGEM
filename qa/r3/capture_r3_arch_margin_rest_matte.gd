...
const OUTPUT_PATH: String = "res://qa/r3/evidence/r3_arch_margin_rest_matte_1600x900.png"
...
func _capture() -> void:
	...
	if change_scene_to_file("res://scenes/main.tscn") != OK:
		printerr("[ORIGEM_R3_ARCH_MARGIN_REST_MATTE_CAPTURE_ERROR] mundo principal indisponível")
		quit(1)
		return
	await process_frame
	...
	if arch == null or arch.get_node_or_null("R3RepousoMateDaMargem") == null:
		printerr("[ORIGEM_R3_ARCH_MARGIN_REST_MATTE_CAPTURE_ERROR] camada R3 ausente")
		quit(1)
		return
	...
	camera.queue_free()
	if result != OK:
		printerr("[ORIGEM_R3_ARCH_MARGIN_REST_MATTE_CAPTURE_ERROR] falha ao gravar PNG")
		quit(1)
		return
	print("[ORIGEM_R3_ARCH_MARGIN_REST_MATTE_CAPTURE_OK] %s" % OUTPUT_PATH)
	quit(0)
