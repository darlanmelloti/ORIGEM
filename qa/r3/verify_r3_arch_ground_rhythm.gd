...
func _run() -> void:
	...
	if change_error != OK:
		printerr("[ORIGEM_R3_ARCH_GROUND_RHYTHM_ERROR] Não foi possível carregar o mundo principal.")
		quit(1)
		return
	await process_frame
	...
	if arch == null:
		...
		issues.append_array(ARCH_GROUND_RHYTHM_SCRIPT.call("validate", arch) as PackedStringArray)
		var rhythm: Node = arch.get_node_or_null("R3RitmoArqueologicoDoSolo")
		if rhythm != null:
			var contract: Dictionary = rhythm.call("get_contract") as Dictionary
			for prohibited_key: String in ["adds_geometry", "adds_central_slabs", "adds_lights", "adds_vfx", "adds_collision"]:
				if bool(contract.get(prohibited_key, true)):
					issues.append("o ritmo de solo R3 declarou alteração proibida: %s" % prohibited_key)
		...
		if trigger == null:
			...
			if shape == null or shape.size.x < 5.50:
				issues.append("o ritmo de solo R3 reduziu a folga do gatilho central")
	if issues.is_empty():
		print("[ORIGEM_R3_ARCH_GROUND_RHYTHM_OK] Três grupos baixos laterais refinam o ritmo de solo sem lajes centrais ou bloqueio.")
		quit(0)
		return
	for issue: String in issues:
		printerr("[ORIGEM_R3_ARCH_GROUND_RHYTHM_ERROR] %s" % issue)
	quit(1)
