...
func _initialize() -> void:
	...
	var depth: Node = region.get_node_or_null(LIMINAL_NODE_NAME)
	var forest_root: Node = region.get_node_or_null("FlorestaDensaRegional")
	if depth == null:
		_fail("Módulo de profundidade liminar R4 ausente.")
	else:
	...
	if _issues.is_empty():
		print("[ORIGEM_R4_LIMINAL_DEPTH_OK] profundidade, conector, corredor e física R4 preservados.")
		quit(0)
		return
	for issue: String in _issues:
		printerr("[ORIGEM_R4_LIMINAL_DEPTH_ERROR] %s" % issue)
	quit(1)

func _validate_module(depth: Node) -> void:
	if int(depth.get_meta("r4_liminal_depth_adjusted", 0)) <= 0:
		_fail("Nenhuma árvore R4 recebeu profundidade liminar estática.")
	if int(depth.get_meta("r4_liminal_depth_dynamic_lights", -1)) != 0:
...
func _validate_trees(forest_root: Node) -> void:
	...
	for child: Node in forest_root.get_children():
		if not child is Node3D or not child.has_meta("r4_liminal_depth_static"):
			continue
		...
		if tree.position.z < BAND_START_Z or tree.position.z > BAND_END_Z:
			_fail("Árvore %s excede a faixa liminar R4." % tree.name)
		...
		adjusted += 1
	if adjusted <= 0:
		_fail("Nenhuma árvore R4 foi marcada pela profundidade liminar.")
