...
func _initialize() -> void:
	...
	if silhouette == null:
		_fail("Módulo de silhueta lateral R4 ausente.")
	else:
	...
	if _issues.is_empty():
		print("[ORIGEM_R4_TRAIL_SILHOUETTE_OK] silhueta, corredor, clareira e física R4 preservados.")
		quit(0)
		return
	for issue: String in _issues:
		printerr("[ORIGEM_R4_TRAIL_SILHOUETTE_ERROR] %s" % issue)
	quit(1)

func _validate_module(silhouette: Node) -> void:
	if int(silhouette.get_meta("r4_trail_silhouette_adjusted", 0)) <= 0:
		_fail("Nenhuma copa R4 recebeu organização estática de silhueta.")
	if int(silhouette.get_meta("r4_trail_silhouette_dynamic_lights", -1)) != 0:
...
func _validate_canopies(clusters_root: Node) -> void:
	...
	for child: Node in clusters_root.get_children():
		if not child is Node3D or not child.has_meta("r4_trail_silhouette_static"):
			continue
		...
		if canopy.position.z < BAND_START_Z or canopy.position.z > BAND_END_Z:
			_fail("Copa %s excede a faixa de silhueta R4." % canopy.name)
		...
		adjusted += 1
	if adjusted <= 0:
		_fail("Nenhuma copa R4 foi marcada pela silhueta lateral.")
