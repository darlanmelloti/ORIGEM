...
func _initialize() -> void:
	...
	var reading: Node = region.get_node_or_null(READING_NODE_NAME)
	var forest_root: Node = region.get_node_or_null("FlorestaDensaRegional")
	if reading == null:
		_fail("Módulo de leitura de retorno R4 ausente.")
	else:
	...
	if _issues.is_empty():
		print("[ORIGEM_R4_RETURN_READING_OK] retorno, corredor, conector e física R4 preservados.")
		quit(0)
		return
	for issue: String in _issues:
		printerr("[ORIGEM_R4_RETURN_READING_ERROR] %s" % issue)
	quit(1)

func _validate_module(reading: Node) -> void:
	if int(reading.get_meta("r4_return_reading_adjusted", 0)) != 1:
		_fail("A leitura de retorno deve ajustar exactamente uma árvore R4 existente.")
	if int(reading.get_meta("r4_return_reading_dynamic_lights", -1)) != 0:
...
func _validate_trees(forest_root: Node) -> void:
	...
	for child: Node in forest_root.get_children():
		if not child is Node3D or not child.has_meta("r4_return_reading_static"):
			continue
		...
		if tree.position.z < BAND_START_Z or tree.position.z > BAND_END_Z:
			_fail("Árvore %s excede a faixa de retorno R4." % tree.name)
		...
		adjusted += 1
	if adjusted != 1:
		_fail("A leitura de retorno não marcou exactamente uma árvore R4.")
