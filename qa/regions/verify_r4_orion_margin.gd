...
func _initialize() -> void:
	...
	if margin == null:
		_fail("Módulo de margem Orion R4 ausente.")
	else:
	...
	if _issues.is_empty():
		print("[ORIGEM_R4_ORION_MARGIN_OK] margem, clareira, corredor e física R4 preservados.")
		quit(0)
		return
	for issue: String in _issues:
		printerr("[ORIGEM_R4_ORION_MARGIN_ERROR] %s" % issue)
	quit(1)

func _validate_module(margin: Node) -> void:
	if int(margin.get_meta("r4_orion_margin_adjusted", 0)) != 1:
		_fail("A margem Orion deve ajustar exactamente uma árvore R4 existente.")
	if int(margin.get_meta("r4_orion_margin_dynamic_lights", -1)) != 0:
...
func _validate_trees(forest_root: Node) -> void:
	...
	for child: Node in forest_root.get_children():
		if not child is Node3D or not child.has_meta("r4_orion_margin_static"):
			continue
		...
		if tree.position.z < BAND_START_Z or tree.position.z > BAND_END_Z:
			_fail("Árvore %s excede a margem Orion R4." % tree.name)
		...
		adjusted += 1
	if adjusted != 1:
		_fail("A margem Orion não marcou exactamente uma árvore R4.")
