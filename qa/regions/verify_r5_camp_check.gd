...
func _verify() -> void:
	...
	var count := 0
	var valid := camp != null and bool(camp.get_meta("r5_check_applied", false)) and int(camp.get_meta("r5_check_dynamic_lights", -1)) == 0
	if camp != null:
		for node: Node in camp.get_children():
			if node is MeshInstance3D and bool(node.get_meta("r5_check_static", false)) and bool(node.get_meta("r5_consistency_static", false)):
	...
	if valid and count == 2:
		print("[ORIGEM_R5_CHECK_OK] duas caixas existentes ajustadas, colisores preservados e zero luzes."); quit(); return
	printerr("[ORIGEM_R5_CHECK_ERROR] caixas=%d" % count); quit(1)
