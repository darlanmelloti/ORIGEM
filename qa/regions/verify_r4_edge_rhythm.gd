...
func _initialize() -> void:
	...
	if rhythm == null:
		_fail("Módulo de cadência de borda R4 ausente.")
	else:
	...
	if _issues.is_empty():
		print("[ORIGEM_R4_EDGE_RHYTHM_OK] borda, conector, corredor e clareira R4 preservados.")
		quit(0)
		return
	for issue: String in _issues:
		printerr("[ORIGEM_R4_EDGE_RHYTHM_ERROR] %s" % issue)
	quit(1)

func _validate_module(rhythm: Node) -> void:
	if int(rhythm.get_meta("r4_edge_rhythm_adjusted", 0)) <= 0:
		_fail("Nenhuma pedra R4 recebeu cadência estática de borda.")
	if int(rhythm.get_meta("r4_edge_rhythm_dynamic_lights", -1)) != 0:
...
func _validate_stones(details_root: Node) -> void:
	...
	for child: Node in details_root.get_children():
		...
		if not stone.has_meta("r4_edge_rhythm_static"):
			continue
		...
		if stone.position.z < BAND_START_Z or stone.position.z > BAND_END_Z:
			_fail("Pedra %s excede a faixa de borda R4." % stone.name)
		if abs(stone.position.x - _path_x(stone.position.z)) < MIN_PATH_CLEARANCE:
			_fail("Pedra %s invade o corredor ou conector." % stone.name)
		adjusted += 1
	if adjusted <= 0:
		_fail("Nenhuma pedra R4 foi marcada pela cadência de borda.")
