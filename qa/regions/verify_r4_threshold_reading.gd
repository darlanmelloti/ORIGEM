...
func _initialize() -> void:
	...
	var reading: Node = region.get_node_or_null(THRESHOLD_NODE_NAME)
	var details_root: Node = region.get_node_or_null("RaizesPedrasESinaisP0")
	if reading == null:
		_fail("Módulo de leitura do limiar R4 ausente.")
	else:
	...
	if _issues.is_empty():
		print("[ORIGEM_R4_THRESHOLD_OK] limiar, conector, corredor e clareira R4 preservados.")
		quit(0)
		return
	for issue: String in _issues:
		printerr("[ORIGEM_R4_THRESHOLD_ERROR] %s" % issue)
	quit(1)

func _validate_module(reading: Node) -> void:
	if int(reading.get_meta("r4_threshold_reading_adjusted", 0)) <= 0:
		_fail("Nenhuma raiz R4 recebeu leitura estática de limiar.")
	if int(reading.get_meta("r4_threshold_reading_dynamic_lights", -1)) != 0:
...
func _validate_roots(details_root: Node) -> void:
	...
	for child: Node in details_root.get_children():
		...
		if not root_mesh.has_meta("r4_threshold_reading_static"):
			continue
		if root_mesh.position.z < BAND_START_Z or root_mesh.position.z > BAND_END_Z:
			_fail("Raiz %s excede a faixa do limiar R4." % root_mesh.name)
		if abs(root_mesh.position.x - _path_x(root_mesh.position.z)) < MIN_PATH_CLEARANCE:
			_fail("Raiz %s invade o corredor ou conector." % root_mesh.name)
		adjusted += 1
	if adjusted <= 0:
		_fail("Nenhuma raiz R4 foi marcada pela leitura do limiar.")
