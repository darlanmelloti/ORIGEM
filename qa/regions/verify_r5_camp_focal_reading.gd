...
func _verify() -> void:
	...
	var count := 0
	var valid := camp != null and bool(camp.get_meta("r5_focal_reading_applied", false)) and int(camp.get_meta("r5_focal_reading_dynamic_lights", -1)) == 0
	if camp != null:
		for node: Node in camp.get_children():
			if node is MeshInstance3D and bool(node.get_meta("r5_focal_reading_static", false)) and String(node.name).begins_with("CaixaDeCampo_"):
	...
	if valid and count == 2:
		print("[ORIGEM_R5_FOCAL_OK] duas caixas visuais existentes ajustadas, zero luzes e sem colisores alterados."); quit(); return
	printerr("[ORIGEM_R5_FOCAL_ERROR] caixas=%d" % count); quit(1)
