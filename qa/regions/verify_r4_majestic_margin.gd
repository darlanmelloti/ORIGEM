...
func _initialize() -> void:
	...
	var margin: Node = region.get_node_or_null(MARGIN_NODE_NAME)
	var forest_root: Node = region.get_node_or_null("FlorestaDensaRegional")
	if margin == null:
		_fail("Módulo de margem Majestic R4 ausente.")
	else:
		_validate_module(margin)
	if forest_root == null:
	...
	if _issues.is_empty():
		print("[ORIGEM_R4_MAJESTIC_MARGIN_OK] margem, corredor, conector e física R4 preservados.")
		quit(0)
		return
	for issue: String in _issues:
		printerr("[ORIGEM_R4_MAJESTIC_MARGIN_ERROR] %s" % issue)
	quit(1)

func _validate_module(margin: Node) -> void:
	if int(margin.get_meta("r4_majestic_margin_adjusted", 0)) != 1:
		_fail("A margem Majestic deve ajustar exactamente uma árvore R4 existente.")
	if int(margin.get_meta("r4_majestic_margin_dynamic_lights", -1)) != 0:
...
func _validate_trees(forest_root: Node) -> void:
	...
	for child: Node in forest_root.get_children():
		if not child is Node3D or not child.has_meta("r4_majestic_margin_static"):
			continue
		...
		if tree.position.z < BAND_START_Z or tree.position.z > BAND_END_Z:
			_fail("Árvore %s excede a margem Majestic R4." % tree.name)
		...
		adjusted += 1
	if adjusted != 1:
		_fail("A margem Majestic não marcou exactamente uma árvore R4.")
