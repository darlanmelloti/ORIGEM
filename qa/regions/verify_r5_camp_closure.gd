...
func _verify() -> void:
	...
	var count := 0
	var valid := camp != null and bool(camp.get_meta("r5_closure_applied", false)) and int(camp.get_meta("r5_closure_dynamic_lights", -1)) == 0
	if camp != null:
		for node: Node in camp.get_children():
			if node is MeshInstance3D and bool(node.get_meta("r5_closure_static", false)) and bool(node.get_meta("r5_rhythm_final_static", false)):
	...
	if valid and count == 2:
		print("[ORIGEM_R5_CLOSURE_OK] duas caixas existentes ajustadas, colisores preservados e zero luzes."); quit(); return
	printerr("[ORIGEM_R5_CLOSURE_ERROR] caixas=%d" % count); quit(1)
