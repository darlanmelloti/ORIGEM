...
func _initialize() -> void:
	...
	var silhouette: Node = region.get_node_or_null(SILHOUETTE_NODE_NAME)
	var forest_root: Node = region.get_node_or_null("FlorestaDensaRegional")
	if silhouette == null:
		_fail("Módulo de silhueta de entrada R4 ausente.")
	else:
		_validate_module(silhouette)
	if forest_root == null:
	...
	if _issues.is_empty():
		print("[ORIGEM_R4_ENTRY_SILHOUETTE_OK] entrada, clareira, corredor e física R4 preservados.")
		quit(0)
		return
	for issue: String in _issues:
		printerr("[ORIGEM_R4_ENTRY_SILHOUETTE_ERROR] %s" % issue)
	quit(1)

func _validate_module(silhouette: Node) -> void:
	if int(silhouette.get_meta("r4_entry_silhouette_adjusted", 0)) != 1:
		_fail("A silhueta de entrada deve ajustar exactamente uma árvore R4 existente.")
	if int(silhouette.get_meta("r4_entry_silhouette_dynamic_lights", -1)) != 0:
...
func _validate_trees(forest_root: Node) -> void:
	...
	for child: Node in forest_root.get_children():
		if not child is Node3D or not child.has_meta("r4_entry_silhouette_static"):
			continue
		...
		if tree.position.z < BAND_START_Z or tree.position.z > BAND_END_Z:
			_fail("Árvore %s excede a faixa de entrada R4." % tree.name)
		...
		adjusted += 1
	if adjusted != 1:
		_fail("A silhueta de entrada não marcou exactamente uma árvore R4.")
