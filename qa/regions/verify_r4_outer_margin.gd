...
func _initialize() -> void:
	...
	var margin: Node = region.get_node_or_null(OUTER_MARGIN_NODE_NAME)
	var forest_root: Node = region.get_node_or_null("FlorestaDensaRegional")
	if margin == null:
		_fail("Módulo de margem exterior R4 ausente.")
	else:
	...
	if _issues.is_empty():
		print("[ORIGEM_R4_OUTER_MARGIN_OK] margem, conector, corredor e física R4 preservados.")
		quit(0)
		return
...
func _validate_module(margin: Node) -> void:
	if int(margin.get_meta("r4_outer_margin_adjusted", 0)) <= 0:
		_fail("Nenhuma árvore R4 recebeu refinamento estático de margem.")
...
func _validate_trees(forest_root: Node) -> void:
	...
	for child: Node in forest_root.get_children():
		if not child is Node3D or not child.has_meta("r4_outer_margin_static"):
			continue
		...
		adjusted += 1
	if adjusted <= 0:
		_fail("Nenhuma árvore R4 foi marcada pela margem exterior.")
